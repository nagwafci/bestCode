    ...
    return Container(
      ...
      child: IconButton(
        ...
        onPressed: () async {
          ...
          try {
            if (wishlistProvider.getWishlistItems
                .containsKey(widget.storeName)) {
              wishlistProvider.removeWishlistItemFromFirebase(
                wishlistId:
                    wishlistProvider.getWishlistItems[widget.storeName]!.id,
                storeName: widget.storeName,
              );
            } else {
              wishlistProvider.addToWishlistFirebase(
                  storeName: widget.storeName, context: context);
            }
        ...
        icon: isLoading
            ...
            : Icon(
                wishlistProvider.isStoreInWishlist(storeName: widget.storeName)
                    ? IconlyBold.heart
                    : IconlyLight.heart,
                size: widget.size,
                color: wishlistProvider.isStoreInWishlist(
                        storeName: widget.storeName)
                    ? Colors.red
                    : Colors.grey,
              ),
